...
var w2utils = (function () {
    ...
    var obj = {
        ...
        settings : {
            ...
            "phrases"           : {
                "Add new record": "新規レコードの追加",
                "Add New": "追加",
                "AJAX error. See console for more details.": "AJAXエラー。javascriptコンソールを参照してください。",
                "All Fields": "全フィールド",
                "Are you sure you want to delete selected records?": "選択したレコードを削除してもよろしいですか？",
                "Attach files by dragging and dropping or Click to Select": "ドラッグアンドドロップかクリックしてファイルを選択してください。",
                "begins": "前方一致",
                "between": "範囲",
                "buffered": "読込済み",
                "Clear Search": "検索をクリア",
                ...
                "Delete selected records": "選択したレコードの削除",
                "Delete": "削除",
                "Edit": "編集",
                "ends": "後方一致",
                "Hide": "隠す",
                ...
                "Load": "読み込み",
                "More": "さらに",
                "Multi Fields": "マルチフィールド",
                "Multiple Fields": "複数フィールド",
                "No items found": "見つかりませんでした。",
                "No": "いいえ",
                "none": "なし",
                "Not a float": "小数値ではりません",
                "Not a hex number": "16進数ではありませｎ",
                "Not a valid date": "正しい日付ではありません",
                "Not a valid email": "Emailアドレスではありません",
                "Not alpha-numeric": "英文字ではありません",
                "Not an integer": "整数値ではありません",
                "Not in money format": "金額ではありません",
                "Notification": "通知",
                "of": "全",
                "Ok": "Ok",
                "Open Search Fields": "検索フィールドを開く",
                "Records": "レコード",
                "Refreshing...": "検索中...",
                "Reload data in the list": " データを更新する",
                "Remove": "削除の確認",
                "Required field": "必須項目",
                "Reset Column Size": "カラムサイズをリセット",
                "Reset": "リセット",
                "Return data is not in JSON format. See console for more information.": "応答データが正しいJSON形式ではありません。",
                "Returned data is not in valid JSON format.": "応答データが正しいJSON形式ではありません。",
                "Save changed records": "変更したレコードの保存",
                "Save": "保存",
                "Saving...": "保存中…",
                "Search": "検索",
                "Search...": "検索...",
                "Select Search Field": "検索項目の選択",
                "selected": "レコード選択",
                "Server Response": "サーバー応答",
                "Show": "表示",
                "Show/hide columns": "カラムの表示/非表示",
                "Skip": "スキップ",
                "Toggle Line Numbers": "行番号表示切り替え",
                "Yes": "Yes",
                "Yesterday": "昨日"
            }
...
(function () {
    ...
    w2grid.prototype = {
        ...
        addFirst: function (record, first) {
            if (!$.isArray(record)) record = [record];
            var added = 0;
                    ...
                    for (var s in this.searchData) {
                        ...
                        switch (sdata.operator) {
                            ...
                            case 'between':
                                ...
                                } else if('text' == search.type) {
                                    if (rec[search.field] >= val2 && rec[search.field] <= val3) fl++;
                                }
                                if (search.type == 'date') {
        ...
        select: function () {
            ...
            if (!this.multiSelect && !this.singleSelect) this.selectNone();
            for (var a = 0; a < arguments.length; a++) {
        ...
        selectAll: function () {
            if (this.multiSelect === false || this.selectAllEnable === false) return;
            // event before
                    ...
                    if (field.toLowerCase() == 'all') {
                        ...
                        if (this.searches.length > 0) {
                            for (var s in this.searches) {
                                ...
                                if (['int', 'float', 'money', 'currency', 'percent', 'text'].indexOf(search.type) != -1 && String(value).indexOf('-') != -1) {
                        ...
                        if (value != '') {
                            var op  = 'contains';
                            var val = value;
                            if (['date', 'time', 'list', 'text'].indexOf(search.type) != -1) op = 'is';
        ...
        resizeRecords: function () {
            ...
            if (!this.fixedBody) {
                ...
                grid.css('height', obj.height + 18);
                body.css('height', calculatedHeight + 18);
        ...
        getSearchesHTML: function () {
            ...
            for (var i = 0; i < this.searches.length; i++) {
                ...
                if (['int', 'float', 'money', 'currency', 'percent', 'date', 'time', 'text'].indexOf(s.type) != -1) {
        ...
        getColumnsHTML: function () {
            ...
            function getColumns (master) {
                ...
                if (obj.show.selectColumn) {
                    html += '<td class="w2ui-head w2ui-col-select" '+
                            '        onclick="w2ui[\''+ obj.name +'\'].getSelection().length > 0 ? w2ui[\''+ obj.name +'\'].selectNone() : checkColumn(w2ui[\''+ obj.name +'\'])">'+
                            '</td>';
                }
                if (obj.show.expandColumn) {
                    html += '<td class="w2ui-head w2ui-col-expand">'+
...
(function () {
    var w2form = function(options) {
        ...
        this.msgNotJSON    = w2utils.lang('通信に失敗しました。');
        this.msgAJAXerror  = w2utils.lang('通信に失敗しました。');
        this.msgRefresh    = w2utils.lang('読み込み中です');
